...
在交互式终端中运行时，安装器会询问 Harness agent 模型配置，包括使用同一模型、分别配置 planner/executor/verifier/debugger，或跳过模型配置。CI 或无 TTY 环境会跳过交互；可用环境变量指定：
...
In an interactive terminal, the installer asks how to configure Harness agent models: one shared model, separate models for planner/executor/verifier/debugger, or skipped model configuration. CI and non-TTY environments skip prompts; use environment variables for unattended installs:
